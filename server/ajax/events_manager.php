...
if(isset($_POST['update-event']) && $_POST['action'] == 'PEC_CREATE_EVENT'){
    ...
    if((int)$_POST['update-event'] > 0){
        ...
        if(isset($_POST['selected_calendars']) && count($_POST['selected_calendars']) > 0){
            ...
            //==== Reminder Settings
            $reminderObj = new C_Reminder($eventObj->id,$guests,$reminder_type,$reminder_time,$reminder_time_unit);
            $params['guests'] = $reminderObj->guests;

            //==== save reminders
            if(isset($reminderObj->reminder_type) && count($reminderObj->reminder_type) > 0){
                $reminderObj->updateReminders();
            }
            //==== Save guests
            if($reminderObj->guests){
                $reminderObj->updateGuests();
            }
...
if(isset($_POST['action']) && $_POST['action'] == 'LOAD_SINGLE_EVENT_BASED_ON_EVENT_ID'){
    ...
    //=== load reminder data if any
    $reminderData = C_Reminder::loadReminders($_POST['eventID']);
    if($reminderData != NULL) $eventData = array_merge($eventData,array('reminderData'=>$reminderData));

    $reminderGuests = C_Reminder::loadGuests($_POST['eventID']);
    if($reminderGuests != NULL) $eventData = array_merge($eventData,array('reminderGuests'=>$reminderGuests));
